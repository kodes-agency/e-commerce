<script lang="ts">
  import { subscribe } from "$lib/store/store";
...
<footer class="bg-[var(--black-color)] px-10 pb-4 mt-20">
  <section
    class="flex flex-col md:flex-row space-y-8 md:space-y-0 justify-between py-20 md:py-10"
  >
    ...
    >
      ...
      <div class="flex flex-col space-y-2 text-center md:text-end">
        <a
          href={$page.data.footer?.data?.menu?.data?.attributes?.phoneLink}
          target="_blank"
          class="text-[var(--white-color)] text-md footer-links transition-all"
          >{$page.data.footer?.data?.menu?.data?.attributes?.phoneName}</a
        >
        <a
          href={$page.data.footer?.data?.menu?.data?.attributes?.emailLink}
          target="_blank"
          class="text-[var(--white-color)] text-md footer-links transition-all"
          >{$page.data.footer?.data?.menu?.data?.attributes?.emailName}</a
        >
      </div>
      <div class="flex flex-col space-y-2 text-center md:text-end">
        <a
      ...
      <div class="subsription-wrapper" bind:this={subscriptonWrapper}>
        ...
        {#if $subscribe}
          <p class="italic font-bold">Вие сте абониран успешно!</p>
        {:else}
          <div
            class="flex flex-col md:flex-row items-center justify-center space-y-3 md:space-y-0 md:space-x-3"
          >
            <input
              class="bg-[var(--black-color)] border border-[var(--white-color)] rounded-none text-[var(--white-color)] text-center text-md h-8 w-56"
              type="text"
